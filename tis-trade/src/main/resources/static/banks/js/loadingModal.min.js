!function (s, i, c, t) {
  ...
  function e(c, t) {
      ...
      rotatingPlane: {
        ...
        setBackground: function (i) {
          e.animationBox.find("*").each(function (c, t) {
            s(t).css("background-color") && "rgba(0, 0, 0, 0)" != s(t).css(
                "background-color") && s(t).css("background-color", i)
          })
        }
      },
      ...
      circle: {
        ...
        setBackground: function (c) {
          e.animationBox.children().find("*").each(function (t, e) {
            "rgba(0, 0, 0, 0)" !== i.getComputedStyle(e,
                ":before").getPropertyValue("background-color") && s(
                "body").append(
                s("<style data-custom-style>." + s(e).attr("class").split(
                        " ")[0] + ":before {background-color: " + c
                    + " !important;}</style>"))
          })
        }
      },
    ...
    }, this.settings = s.extend({}, l,
        t), this.modal = null, this.modalText = null, this.animationBox = null, this.modalBg = null, this.currenAnimation = null, this.init(), this
  }
  ...
  s.extend(e.prototype, {
    init: function () {
      var i = s(
              '<div class="jquery-loading-modal jquery-loading-modal--visible"></div>'),
      ...
      "" !== this.settings.text ? d.html(this.settings.text)
          : d.hide(), this.currenAnimation = this.animations[this.settings.animation], t.append(
          this.currenAnimation.html), e.append(t).append(d), i.append(
          c), i.append(e), "auto" === this.settings.position && "body"
      !== this.element.tagName.toLowerCase() ? (i.css("position",
          "absolute"), s(this.element).css("position", "relative")) : "auto"
          !== this.settings.position && s(this.element).css("position",
              this.settings.position), s(this.element).append(
          i), this.modalBg = c, this.modal = i, this.modalText = d, this.animationBox = t, this.color(
          this.settings.color), this.backgroundColor(
          this.settings.backgroundColor), this.opacity(this.settings.opacity)
    }, hide: function () {
      var s = this.modal;
      s.removeClass("jquery-loading-modal--visible").addClass(
          "jquery-loading-modal--hidden"), setTimeout(function () {
        s.hide()
      }, 1e3)
    }, backgroundColor: function (s) {
      this.modalBg.css({"background-color": s})
    }, color: function (c) {
      s("[data-custom-style]").remove(), this.modalText.css("color",
          c), this.currenAnimation.setBackground
          ? this.currenAnimation.setBackground(c)
          : this.animationBox.children().find("*").each(function (t, e) {
            s(e).css("background-color") && "rgba(0, 0, 0, 0)" != s(e).css(
                "background-color") && s(e).css("background-color",
                c), "rgba(0, 0, 0, 0)" !== i.getComputedStyle(e,
                ":before").getPropertyValue("background-color") && s(
                "body").append(
                s("<style data-custom-style>." + s(e).attr("class").split(
                        " ")[0] + ":before {background-color: " + c
      ...
      this.modal.show().removeClass("jquery-loading-modal--hidden").addClass(
          "jquery-loading-modal--visible")
    }, animation: function (s) {
      this.animationBox.html(
          ""), this.currenAnimation = this.animations[s], this.animationBox.append(
          this.currenAnimation.html)
    }, destroy: function () {
    ...
    if (i === t || "object" == typeof i) {
      return this.each(function () {
        s.data(this, "plugin_" + d) || s.data(this, "plugin_" + d,
            new e(this, i))
      });
    }
    if ("string" == typeof i && "_" !== i[0] && "init" !== i) {
      ...
      return this.each(function () {
        ...
        t instanceof e && "function" == typeof t[i] && (l = t[i].apply(t,
            Array.prototype.slice.call(c, 1))), "destroy" === i && s.data(this,
            "plugin_" + d, null)
      }), l !== t ? l : this
    }
  }
